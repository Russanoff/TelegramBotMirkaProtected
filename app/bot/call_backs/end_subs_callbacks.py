...
from app.bot.inline_menu.end_subs_menu import step_two, step_one, step_two_crypto, step_two_stars
...
@end_subs_callbacks.callback_query(F.data == "stars_pay")
async def stars_variant(callback: CallbackQuery):
    await callback.answer('Оплата Telegram Stars')
    await callback.message.edit_text(text="Выберите срок продления доступа:", reply_markup=step_two_stars)
